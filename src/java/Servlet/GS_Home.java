    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            for (int x = 0; x < meetingList.size(); x++) {
                meetingProjects.add(gsdao.getBasicProjectDetails(meetingList.get(x).getProjectID()).getName());
            }
            
            OCPDDAO oc = new OCPDDAO();
            for (int x = 0; x < unconfirmedMeeting.size(); x++) {
                unconfirmedMProjects.add(gsdao.getBasicProjectDetails(unconfirmedMeeting.get(x).getProjectID()));
                unconfirmedMeeting.get(x).setTasks(gsdao.getAgenda(unconfirmedMeeting.get(x)));
            }
